        ...
        <div class="column col-3">
            <nav class="sidebar">
                <ul class="nav flex-column p-3">
                    ...
                    <form action="${pageContext.request.contextPath}/productSearch" class="form-outline input-group input-group-sm mt-2 px-3 rounded" method="post">
                    ...
                    <c:choose>
                        <c:when test="${empty searchedProducts}">
                            <tr>
                                ...
                                <td>
                                    ${searchedProduct.amount}
                                </td>
                                <td>
                                    ${searchedProduct.amountType}
                                </td>
                                <td>
                                    ${searchedProduct.type}
                                </td>
                                <td>
                                    ${searchedProduct.price} Ft
                                </td>
                                <td>
                                    ${searchedProduct.orderStatus}
                                </td>
                                <td>
                                    ${searchedProduct.warehouseStatus}
                                </td>
                                <c:if test="${sessionScope.adminRoleType == 'true' || sessionScope.warehouseRoleType == 'true'}">
                                    <td>
                                        <a onclick='return confirm("Biztosan törölni szeretné a terméket?");' href="${pageContext.request.contextPath}/product/delete?id=${searchedProduct.id}"  class="btn btn-danger sm">Törlés</a>
                                    </td>
                                    <td>
                                        <a href="${pageContext.request.contextPath}/product/productUpdate?id=${searchedProduct.id}" class="btn btn-warning sm">Módosítás</a>
                                    </td>
                                </c:if>
                            </tr>
                        </c:when>
                        <c:otherwise>
                            <c:forEach items="${searchedProducts}" var="sp">
                                <tr>
                                    <td>
                                        ${sp.productName}
                                    </td>
                                    ...
                                    <c:if test="${sessionScope.adminRoleType == 'true' || sessionScope.warehouseRoleType == 'true'}">
                                        <td>
                                            <a onclick='return confirm("Biztosan törölni szeretné a terméket?");' href="${pageContext.request.contextPath}/product/delete?id=${sp.id}"  class="btn btn-danger sm">Törlés</a>
                                        </td>
                                        <td>
                                            <a href="${pageContext.request.contextPath}/product/productUpdate?id=${sp.id}" class="btn btn-warning sm">Módosítás</a>
                                        </td>
                                    </c:if>
                                </tr>
                            </c:forEach>
